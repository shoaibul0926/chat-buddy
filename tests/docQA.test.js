...
test('(b) an on-topic question returns the correct, relevant sentence', () => {
  const files = sampleFiles();
  const match = findAnswer(files, 'How many days do I have to get a refund?');
...
test('(b) a second on-topic question matches a different, correct sentence', () => {
  const files = sampleFiles();
  const match = findAnswer(files, 'Where are your offices located?');
  assert.ok(match, 'expected a match for an on-topic office-location question');
  assert.match(match.sentence, /Springfield/);
});

test('(c) an out-of-document question returns no answer instead of a weak/unrelated match', () => {
  const files = sampleFiles();
  ...
  const match = findAnswer(files, "What's the weather like tomorrow?");
  assert.equal(match, null);
});

test('regression: a single incidental keyword match is rejected for multi-keyword questions', () => {
  const files = [{
    name: 'doc.txt',
    text: [
  ...
  // relevance threshold.
  const match = findAnswer(files, 'What is the performance budget review?');
...
test('multi-keyword question with strong overlap still matches', () => {
  const files = sampleFiles();
  const match = findAnswer(files, 'Is shipping cost refundable?');
  assert.ok(match);
